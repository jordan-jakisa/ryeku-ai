...
export const fetchSources = async (topic: ResearchTopic): Promise<Source[]> => {
  ...
  // FastAPI treats list query parameters as repeated keys and marks them as REQUIRED.
  // If the array is empty the backend returns 422. To avoid this we always send at
  // least one placeholder value so the parameter is present in the query string.
  const focusList = topic.focus.length > 0 ? topic.focus : ["general"];
  focusList.forEach((f) => params.append("focus", f));

  const sourceTypesList =
    topic.sourceTypes.length > 0 ? topic.sourceTypes : ["academic"];
  sourceTypesList.forEach((s) => params.append("sourceTypes", s.toString()));

  const response = await fetch(
    `${API_BASE_URL}/research/sources?${params.toString()}`
  );
