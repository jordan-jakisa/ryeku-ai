...
import { ResearchTopic } from '@/app/page';

interface ResearchFormProps {
  onSubmit: (topic: ResearchTopic) => void;
}

export function ResearchForm({ onSubmit }: ResearchFormProps) {
  const [topic, setTopic] = useState('');
  const [depth, setDepth] = useState<'basic' | 'comprehensive' | 'expert'>('comprehensive');
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="text-center space-y-4">
        ...
        <h1 className="text-4xl font-bold text-white">AI-Powered Deep Research</h1>
        <p className="text-xl text-gray-300 max-w-2xl mx-auto">
          Enter your research topic and let our AI analyze authoritative sources to generate comprehensive insights
        </p>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              <Label className="text-white font-medium">Research Focus Areas (Optional)</Label>
                ...
                {focusAreas.map((area) => (
                  <div key={area.id} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleFocusChange(area.id, checked as boolean)}
